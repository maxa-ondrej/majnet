...
pub fn render_acl(people: &PeopleFile, projects: &[(String, ProjectConfig)]) -> serde_json::Value {
    let admins: Vec<&str> = people.people.iter().filter(|p| p.admin).map(|p| p.tailscale.as_str()).collect();
    ...
    let mut acls = vec![json!({ "action": "accept", "src": ["group:admins"], "dst": ["tag:main:*"] })];

    for (name, project) in projects {
        let members: Vec<&str> = project
            .members
            .iter()
            .filter_map(|m| people.people.iter().find(|p| p.github.eq_ignore_ascii_case(&m.user)))
        ...
        acls.push(json!({ "action": "accept", "src": ["group:admins"], "dst": [format!("{tag}:*")] }));
    }
...
pub async fn sync_acl(state: &AppState, people: &PeopleFile, projects: &[(String, ProjectConfig)]) -> Result<()> {
    ...
    let response = state.http.post(&url).bearer_auth(api_key).json(&policy).send().await?;
    let status = response.status();
    anyhow::ensure!(status.is_success(), "Tailscale ACL push failed ({status}): {}", response.text().await.unwrap_or_default());
...
pub async fn authkey(State(state): State<Arc<AppState>>, Path(project): Path<String>) -> Result<String, (StatusCode, String)> {
...
async fn mint_authkey(state: &AppState, project: &str) -> Result<String> {
    let (api_key, tailnet) = ts_credentials(state).context("Tailscale API not configured on the bot")?;
    anyhow::ensure!(
    ...
    let response = state.http.post(&url).bearer_auth(api_key).json(&body).send().await?;
    let status = response.status();
    let payload: serde_json::Value = response.json().await?;
    anyhow::ensure!(status.is_success(), "Tailscale key creation failed ({status}): {payload}");
    let key = payload["key"].as_str().context("key response has no 'key'")?.to_string();
    state.store.log_event("ts-authkey", None, project)?;
    Ok(key)
}

fn ts_credentials(state: &AppState) -> Option<(&str, &str)> {
    let key = state.config.tailscale_api_key.as_deref().filter(|k| !k.is_empty())?;
...
mod tests {
    ...
    fn people() -> PeopleFile {
        PeopleFile {
            people: vec![
                Person { github: "maxa-ondrej".into(), tailscale: "ondrej@example.com".into(), admin: true },
                Person { github: "dev1".into(), tailscale: "dev1@example.com".into(), admin: false },
            ],
        }
    }
    ...
    fn project(name: &str, users: &[&str]) -> (String, ProjectConfig) {
        (
            ...
            ProjectConfig {
                name: name.to_string(),
                members: users.iter().map(|u| Member { user: u.to_string(), role: Role::Developer }).collect(),
                apps: vec![],
            },
        )
    }
    ...
    fn admins_reach_control_plane_members_reach_their_project_only() {
        ...
        assert_eq!(acl["groups"]["group:admins"], serde_json::json!(["ondrej@example.com"]));
        assert_eq!(acl["groups"]["group:proj-zpevnik"], serde_json::json!(["dev1@example.com"]));
        let acls = acl["acls"].as_array().unwrap();
        assert!(acls.iter().any(|r| r["src"][0] == "group:proj-zpevnik" && r["dst"][0] == "tag:proj-zpevnik:*"));
        assert!(acls.iter().any(|r| r["src"][0] == "group:admins" && r["dst"][0] == "tag:main:*"));
    }
