...
- [x] Branded edge error pages: a reconciler-owned `edge-error-pages` nginx on the `edge` network serves MajNet-branded pages instead of raw Traefik errors. Backs a Traefik `errors` middleware (gateway **502/503/504** → 5xx page; applied fleet-wide via the `websecure` entrypoint in traefik.yaml) and a lowest-priority catch-all router (**unrouted host → branded 404**). Scoped to gateway errors only — app-owned 500s/4xx/APIs pass through untouched. Config in reconciler-generated `dynamic/edge-errors.yaml`
...
- [ ] OTEL-native observability (**ADR 0023**, planned): standardize on OTLP at the app boundary — an `otel: on` field on `AppDecl` opts an app in, and the reconciler auto-injects `OTEL_EXPORTER_OTLP_ENDPOINT` + `OTEL_RESOURCE_ATTRIBUTES` (like `DATABASE_URL`). Infra metrics stay native/pull (ADR 0017); traces + structured logs go OTEL/push through a shared **OTEL Collector gateway** to **Tempo + Loki + Grafana** run as internal service apps (ADR 0021, no metrics TSDB — Grafana reads MajNet's native metrics). Backend is `exposure: internal` (tailnet-only), with a per-app Observability tab + "Open in Grafana" deep-links. **Phase 1 (app emit-readiness) shipped 2026-07-21** — `otel: true` on the `AppManifest` + reconciler OTLP env injection in `converge_one` (endpoint from `MAJNET_OTLP_ENDPOINT`, inert until set). **Phase 2 (backend) shipped 2026-07-21** — Collector + Tempo + Loki + Grafana as internal service apps on the private node, Grafana on the tailnet, trace round-trip verified. **Cross-node collector reachability** added via `wg_ports: Vec<u16>` on `AppManifest` (reconciler publishes those ports on the node's `wireguard_ip` over the WG mesh) so `otel: true` apps on any node push to the private-node collector. **Phase 3 (dashboard Observability tab) shipped 2026-07-22** — per-app section gated on `otel`, scoped to the selected env: RED tiles (rate/error/p95 from traces + native memory), Traces⇄Logs panel with inline span waterfall + trace_id-tagged logs, Open-in-Grafana deep-link; reconciler `obs.rs` queries Tempo/Loki over the WG mesh (`wg_ports` [3200]/[3100], `MAJNET_TEMPO_ENDPOINT`/`MAJNET_LOKI_ENDPOINT`). Replaces the dying Coolify collector. **Also surfaced on the project-detail page** (2026-07-22) — the same section with a header switcher over the project's `otel`-enabled apps (and their environments), reusing the per-app `obs.rs` endpoints.
- [x] Observable release progress (**ADR 0022**): a live per-release stepper through `committing → tagging → building → published → tracked`. A `release_progress` store table (keyed `(org, app, version)`, version normalized to bare) is advanced by `do_cut`/`submit_draft`/`submit_repo_group` (committing/tagging/building) and by `record` (published, then tracked after `track_stable`); a tag failure marks the stage failed. `GET /api/releases/progress/{org}` (static prefix, avoids the matchit `{org}/{app}` clash) feeds a `ReleaseSteps` stepper on the `/releases` page (fanned out per project, 5 s refetch). Best-effort + cosmetic; terminal rows GC'd after 1 h. Mirrors the import-status precedent (ADR 0010) end to end.
